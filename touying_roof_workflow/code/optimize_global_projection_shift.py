...
def write_surface_geojson(in_path: Path, out_path: Path, surface: str) -> int:
    data = json.loads(in_path.read_text(encoding="utf-8"))
    features = [
        feat
        for feat in data.get("features", [])
        if feat.get("properties", {}).get("surface") == surface
    ]
    payload = {
        "type": "FeatureCollection",
        "name": f"tongji_full_area_projection_{surface}_only",
        "coordinate_system": "SAR image coordinates: x=range column, y=azimuth row",
        "features": features,
    }
    if "sar_brightness_optimization" in data:
        payload["sar_brightness_optimization"] = data["sar_brightness_optimization"]
...
def run(args: argparse.Namespace) -> None:
    ...
    surface_geojson = out_dir / f"{args.date}_full_area_projection_blue_aligned_{args.surface}_only.geojson"
    n_surface_features = write_surface_geojson(out_geojson, surface_geojson, args.surface)
    overlay_png = out_dir / f"{args.date}_full_area_projection_blue_aligned_overlay.png"
    ...
    if args.plot:
        ...
        if default_png.exists():
            default_png.replace(overlay_png)
    ...
    summary = {
        ...
        "surface_only_features": n_surface_features,
        "surface_only_geojson": str(surface_geojson),
        "bottom_only_geojson": str(surface_geojson) if args.surface == "bottom" else "",
...
def main() -> None:
    ...
    parser.add_argument("--out-dir", default=str(TOUYING_DIR / "results" / "blue_aligned"))
    ...
    parser.add_argument("--surface", default="bottom", choices=["roof", "bottom"])
